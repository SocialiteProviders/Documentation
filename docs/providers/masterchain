...
composer require socialiteproviders/masterchain
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Masterchain\\MasterchainExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Masterchain\\MasterchainExtendSocialite@handle',
    ],
];
```
...
```php
'masterchain' => [
    'client_id' => env('MASTERCHAIN_KEY'),
    'client_secret' => env('MASTERCHAIN_SECRET'),
    'redirect' => env('MASTERCHAIN_REDIRECT_URI')
],
```
...
```php
return Socialite::with('masterchain')->redirect();
```
...
return Socialite::with('masterchain')->stateless(false)->redirect();

// to use stateless
return Socialite::with('masterchain')->stateless()->redirect();
```
...
return Socialite::with('masterchain')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Masterchain')->user();
