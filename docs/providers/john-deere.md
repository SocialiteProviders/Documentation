...
composer require socialiteproviders/john-deere
```
...
* The listener that you add for this provider is `'SocialiteProviders\\JohnDeere\\JohnDeereExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\JohnDeere\\JohnDeereExtendSocialite@handle',
    ],
];
```
...
```php
'john-deere' => [
    'client_id' => env('JOHN_DEERE_KEY'),
    'client_secret' => env('JOHN_DEERE_SECRET'),
    'redirect' => env('JOHN_DEERE_REDIRECT_URI')
],
```
...
```php
return Socialite::with('john-deere')->redirect();
```
...
return Socialite::with('john-deere')->stateless(false)->redirect();

// to use stateless
return Socialite::with('john-deere')->stateless()->redirect();
```
...
return Socialite::with('john-deere')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('john-deere')->user();
