...
* The listener that you add for this provider is `'SocialiteProviders\\Gitee\\GiteeExtendSocialite@handle',`.
...
return Socialite::with('Gitee')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Gitee')->stateless()->redirect();
```
...
return Socialite::with('Gitee')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Gitee')->user();
