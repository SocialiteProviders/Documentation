...
* The listener that you add for this provider is `'SocialiteProviders\\Auth0\\Auth0ExtendSocialite@handle',`.
...
return Socialite::with('Auth0')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Auth0')->stateless()->redirect();
```
...
return Socialite::with('Auth0')->setConfig($config)->redirect();
```
