...
* The listener that you add for this provider is `'SocialiteProviders\\ClaveUnica\\ClaveUnicaExtendSocialite@handle',`.
...
* [ClaveUnica docs on how it works](https://claveunica.gob.cl/institucional/como-funciona)
...
return Socialite::with('claveunica')->stateless(false)->redirect();

// to use stateless
return Socialite::with('claveunica')->stateless()->redirect();
```
...
return Socialite::with('claveunica')->setConfig($config)->redirect();
```
