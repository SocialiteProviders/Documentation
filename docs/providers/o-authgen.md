...
* The listener that you add for this provider is `'SocialiteProviders\\OAuthgen\\OAuthgenExtendSocialite@handle',`.
...
'oauthgen' => [
    ...
    'redirect' => env('OAUTHGEN_REDIRECT_URL'),
],
```
...
return Socialite::with('oauthgen')->stateless(false)->redirect();

// to use stateless
return Socialite::with('oauthgen')->stateless()->redirect();
```
...
return Socialite::with('oauthgen')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('oauthgen')->user();
