...
* The listener that you add for this provider is `'SocialiteProviders\\Odnoklassniki\\OdnoklassnikiExtendSocialite@handle',`.
...
return Socialite::with('odnoklassniki')->stateless(false)->redirect();
...
return Socialite::with('odnoklassniki')->setConfig($config)->redirect();
```
