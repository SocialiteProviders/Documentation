...
* The listener that you add for this provider is `'SocialiteProviders\\Flattr\\FlattrExtendSocialite@handle',`.
...
return Socialite::with('flattr')->stateless(false)->redirect();

// to use stateless
return Socialite::with('flattr')->stateless()->redirect();
```
...
return Socialite::with('flattr')->setConfig($config)->redirect();
```
