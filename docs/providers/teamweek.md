...
* The listener that you add for this provider is `'SocialiteProviders\\Teamweek\\TeamweekExtendSocialite@handle',`.
...
```php
'teamweek' => [
    'client_id' => env('TEAMWEEK_KEY'),
    'client_secret' => env('TEAMWEEK_SECRET'),
    'redirect' => env('TEAMWEEK_REDIREC_URI')
],
```
...
return Socialite::with('teamweek')->stateless(false)->redirect();

// to use stateless
return Socialite::with('teamweek')->stateless()->redirect();
```
...
return Socialite::with('teamweek')->setConfig($config)->redirect();
```
