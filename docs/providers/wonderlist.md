...
composer require socialiteproviders/wonderlist 
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Wonderlist\\WonderlistExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Wonderlist\\WonderlistExtendSocialite@handle',
    ],
];
```
...
```php
'wonderlist' => [
    'client_id' => env('WONDERLIST_KEY'),
    'client_secret' => env('WONDERLIST_SECRET'),
    'redirect' => env('WONDERLIST_REDIRECT_URI')
],
```
...
```php
return Socialite::with('wonderlist')->redirect();
```
...
return Socialite::with('wonderlist')->stateless(false)->redirect();

// to use stateless
return Socialite::with('wonderlist')->stateless()->redirect();
```
...
return Socialite::with('wonderlist')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('wonderlist')->user();
