...
* The listener that you add for this provider is `'SocialiteProviders\\Xero\\XeroExtendSocialite@handle',`.
...
return Socialite::with('Xero')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Xero')->stateless()->redirect();
```
...
return Socialite::with('Xero')->setConfig($config)->redirect();
```
