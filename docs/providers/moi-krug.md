...
* The listener that you add for this provider is `'SocialiteProviders\\MoiKrug\\MoiKrugExtendSocialite@handle',`.
...
```php
'moikrug' => [
    'client_id' => env('MOIKRUG_KEY'),
    'client_secret' => env('MOIKRUG_SECRET'),
...
**Note:** MoiKrug supports **only** stateless request.

```php

// to use stateless
return Socialite::with('moikrug')->stateless()->redirect();
...
return Socialite::with('moikrug')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('moikrug')->user();
