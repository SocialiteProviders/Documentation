...
* The listener that you add for this provider is `'SocialiteProviders\\Flexkids\\FlexkidsExtendSocialite@handle',`.
...
```php
// to use stateless
return Socialite::with('flexkids')->stateless()->redirect();
```
...
return Socialite::with('flexkids')->setConfig($config)->redirect();
```
