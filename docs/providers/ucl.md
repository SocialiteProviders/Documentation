...
* The listener that you add for this provider is `'SocialiteProviders\\UCL\\UCLExtendSocialite@handle',`.
...
```php
'uclapi' => [
    'token' => env('UCLAPI_TOKEN'),
    'client_id' => env('UCLAPI_CLIENT_ID'),
    'client_secret' => env('UCLAPI_CLIENT_SECRET'),
    'redirect' => env('UCLAPI_REDIRECT_URL')
],
```

Go to the [UCL API website](https://uclapi.com/dashboard/) to collect the necessary API keys.
...
return Socialite::with('UCL')->stateless(false)->redirect();

// to use stateless
return Socialite::with('UCL')->stateless()->redirect();
```
...
return Socialite::with('UCL')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('UCL')->user();
