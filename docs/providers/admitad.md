...
* The listener that you add for this provider is `'SocialiteProviders\\Admitad\\AdmitadExtendSocialite@handle',`.
...
return Socialite::with('admitad')->stateless(false)->redirect();

// to use stateless
return Socialite::with('admitad')->stateless()->redirect();
```
...
return Socialite::with('admitad')->setConfig($config)->redirect();
```
