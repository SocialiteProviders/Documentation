...
composer require socialiteproviders/opskins
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Opskins\\OpskinsExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Opskins\\OpskinsExtendSocialite@handle',
    ],
];
```
...
```php
'opskins' => [
    'client_id' => env('OPSKINS_CLIENT_ID'),
    'client_secret' => env('OPSKINS_CLIENT_SECRET'),
    'redirect' => env('OPSKINS_REDIRECT_URI')
],
```
...
```php
return Socialite::with('opskins')->redirect();
```
...
return Socialite::with('opskins')->stateless(false)->redirect();

// to use stateless
return Socialite::with('opskins')->stateless()->redirect();
```
...
return Socialite::with('opskins')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('opskins')->user();
