...
* The listener that you add for this provider is `'SocialiteProviders\\Teamleader\\TeamleaderExtendSocialite@handle',`.
...
```php
'teamleader' => [
    'client_id' => env('TEAMLEADER_KEY'),
    'client_secret' => env('TEAMLEADER_SECRET'),
    'redirect' => env('TEAMLEADER_REDIREC_URI')
],
```
...
return Socialite::with('teamleader')->stateless(false)->redirect();

// to use stateless
return Socialite::with('teamleader')->stateless()->redirect();
```
...
return Socialite::with('teamleader')->setConfig($config)->redirect();
```
