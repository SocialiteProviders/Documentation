...
* The listener that you add for this provider is `'SocialiteProviders\\GitHub\\GitHubExtendSocialite@handle',`.
...
return Socialite::with('github')->stateless(false)->redirect();
...
return Socialite::with('github')->setConfig($config)->redirect();
```
