...
* The listener that you add for this provider is `'SocialiteProviders\\GarminConnect\\GarminConnectExtendSocialite@handle',`.
...
return Socialite::with('garmin-connect')->stateless(false)->redirect();

// to use stateless
return Socialite::with('garmin-connect')->stateless()->redirect();
```
...
return Socialite::with('garmin-connect')->setConfig($config)->redirect();
```
