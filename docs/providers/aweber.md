...
* The listener that you add for this provider is `'SocialiteProviders\\Aweber\\AweberExtendSocialite@handle',`.
...
return Socialite::with('Aweber')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Aweber')->stateless()->redirect();
```
...
return Socialite::with('Aweber')->setConfig($config)->redirect();
```
