...
* The listener that you add for this provider is `'SocialiteProviders\\HeadHunter\\HeadHunterExtendSocialite@handle',`.
...
```php
'headhunter' => [
    'client_id' => env('HEADHUNTER_KEY'),
    'client_secret' => env('HEADHUNTER_SECRET'),
    'redirect' => env('HEADHUNTER_REDIRECT_URI'),
    'user_agent' => env('HEADHUNTER_USER_AGENT'),
],
```

**Note:** Provide the name of the application and the contact email of the developer in the User Agent to contact you if necessary.
 
User Agent example: ``Socialite Providers/1.0 (my-app-feedback@example.com)``
...
return Socialite::with('headhunter')->stateless(false)->redirect();

// to use stateless
return Socialite::with('headhunter')->stateless()->redirect();
```
...
return Socialite::with('headhunter')->setConfig($config)->redirect();
```
