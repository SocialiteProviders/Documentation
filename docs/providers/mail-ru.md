...
* The listener that you add for this provider is `'SocialiteProviders\\Mailru\\MailruExtendSocialite@handle',`.
...
return Socialite::with('mailru')->stateless(false)->redirect();

// to use stateless
return Socialite::with('mailru')->stateless()->redirect();
```
...
return Socialite::with('mailru')->setConfig($config)->redirect();
```
