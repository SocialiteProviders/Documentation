...
* The listener that you add for this provider is `'SocialiteProviders\\Bitbucket\\BitbucketExtendSocialite@handle',`.
...
return Socialite::with('bitbucket')->stateless(false)->redirect();

// to use stateless
return Socialite::with('bitbucket')->stateless()->redirect();
```
...
return Socialite::with('bitbucket')->setConfig($config)->redirect();
```
