...
* The listener that you add for this provider is `'SocialiteProviders\\Franceconnect\\FranceconnectExtendSocialite@handle',`.
...
```php
'franceconnect' => [
    'client_id' => env('FRANCECONNECT_KEY'),
    'client_secret' => env('FRANCECONNECT_SECRET'),
...
return Socialite::with('Franceconnect')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Franceconnect')->stateless()->redirect();
```
...
return Socialite::with('Franceconnect')->setConfig($config)->redirect();
```
...
### Logout User From FranceConnect Service Provider

To disconnect a user from FranceConnect service provider you should call generateLogoutURL method to get a token based logout Url

```php
$logout_url = Socialite::driver('Franceconnect')->generateLogoutURL();
