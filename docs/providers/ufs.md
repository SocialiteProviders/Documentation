...
* The listener that you add for this provider is `'SocialiteProviders\\UFS\\UFSExtendSocialite@handle',`.
...
'ufs' => [
    ...
    'dev_mode' => env('UFS_DEV_MODE'), // optional (true or false)
],
```
...
return Socialite::with('UFS')->stateless(false)->redirect();

// to use stateless
return Socialite::with('UFS')->stateless()->redirect();
```
...
return Socialite::with('UFS')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('UFS')->user();
