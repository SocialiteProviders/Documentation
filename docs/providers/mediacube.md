...
* The listener that you add for this provider is `'SocialiteProviders\\MediaCube\\MediaCubeExtendSocialite@handle',`.
...
```php
'mediacube' => [
    'client_id' => env('MEDIACUBE_KEY'),
    'client_secret' => env('MEDIACUBE_SECRET'),
...
return Socialite::with('mediacube')->stateless(false)->redirect();

// to use stateless
return Socialite::with('mediacube')->stateless()->redirect();
```
...
return Socialite::with('mediacube')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('mediacube')->user();
