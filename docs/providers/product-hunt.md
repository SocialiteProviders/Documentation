...
* The listener that you add for this provider is `'SocialiteProviders\\ProductHunt\\ProductHuntExtendSocialite@handle',`.
...
```php
'producthunt' => [
    'client_id' => env('PRODUCTHUNT_KEY'),
    'client_secret' => env('PRODUCTHUNT_SECRET'),
...
return Socialite::with('producthunt')->stateless(false)->redirect();

// to use stateless
return Socialite::with('producthunt')->stateless()->redirect();
```
...
```php
$clientId = "producthun_oauth_client_id";
$clientSecret = "producthun_oauth_clientsecret";
$redirectUrl = "http://yourdomain.com/api/redirect";
$additionalProviderConfig = ['site' => 'meta.stackoverflow.com'];
$config = new \SocialiteProviders\Manager\Config($clientId, $clientSecret, $redirectUrl, $additionalProviderConfig);
return Socialite::with('producthunt')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('producthunt')->user();
