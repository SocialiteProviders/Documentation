...
* The listener that you add for this provider is `'SocialiteProviders\\Sage\\SageExtendSocialite@handle',`.
...
return Socialite::with('Sage')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Sage')->stateless()->redirect();
```
...
return Socialite::with('Sage')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Sage')->user();
