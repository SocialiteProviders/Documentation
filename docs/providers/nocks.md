...
* The listener that you add for this provider is `'SocialiteProviders\\Nocks\\NocksExtendSocialite@handle',`.
...
```php
'nocks' => [
    'client_id'     => env('NOCKS_KEY'),
    'client_secret' => env('NOCKS_SECRET'),
...
return Socialite::with('Nocks')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Nocks')->stateless()->redirect();
```
...
return Socialite::with('Nocks')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Nocks')->user();
