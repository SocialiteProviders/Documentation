...
* The listener that you add for this provider is `'\SocialiteProviders\OSChina\OSChinaExtendSocialite:class',`.
...
return Socialite::with('OSChina')->stateless(false)->redirect();

// to use stateless
return Socialite::with('OSChina')->stateless()->redirect();
```
...
return Socialite::with('OSChina')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('OSChina')->user();
